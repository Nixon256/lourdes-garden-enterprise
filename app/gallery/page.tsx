...
import { X, ChevronLeft, ChevronRight, ZoomIn, Maximize2, Loader2, ImageIcon } from 'lucide-react'
...
export default function GalleryPage() {
        ...
        fetch('/api/gallery')
            ...
            .then(data => {
                if (data.images?.length > 0) {
                    ...
                    // Fallback to static data if DB has no images yet
                    import('./gallery-data').then(m => {
                        setImages(m.galleryImages.map(img => ({
                            id: img.id,
                            url: img.src,
                            alt: img.alt,
    ...
    const content = isMounted ? t[language as keyof typeof t] ?? t.en : t.en
            ...
            {lightboxIndex !== null && images[lightboxIndex] && (
                ...
                >
                    ...
                    <div
                        ...
                        onClick={(e) => { e.stopPropagation(); toggleZoom() }}
                    >
                        <div className={`relative transition-transform duration-500 ${isZoomed ? 'scale-150' : 'scale-100'} w-full h-full p-4 md:p-20 flex items-center justify-center`}>
                            <img
